...
### 在 provider 切换后保持 Codex 线程可见且可恢复
...
切换 `model_provider` 之后，Codex 的线程可见性有时会出问题。

常见现象：

- 原来能看到的历史会话，在换 provider 后突然消失
- 线程其实还在，但侧边栏项目根路径可能丢了
- `codex resume` 和 Codex App 可能表现不一致，因为线程元数据分散在 rollout files、SQLite 和侧边栏状态里

`codex-threadkeeper` 会把这些关键状态一起协调：

- `~/.codex/sessions` 和 `~/.codex/archived_sessions`
- `~/.codex/state_5.sqlite`
- `~/.codex/backups_state/threadkeeper` 下的受管备份

## Windows 图形界面

如果你不想装 Node、也不想进终端，现在可以直接使用发布页里的 `CodexThreadkeeper.exe`。
...
- 从 Releases 下载 `CodexThreadkeeper.exe`
- 打开后先点 `Refresh`
- 选择目标 Provider
- 点击 `Execute`
...
```text
请帮我用 codex-threadkeeper 修复 Codex 历史会话在当前 provider 下不可见的问题。

步骤：
1. 先运行 `codex-threadkeeper status`。
2. 如果当前 provider 已经正确，就运行 `codex-threadkeeper sync`。
3. 如果我明确说要切到某个 provider，就改为运行 `codex-threadkeeper switch <provider-id>`。
4. 如果提示 `state_5.sqlite` 正在被占用，就提醒我先关闭 Codex / Codex App / app-server 再重试。
5. 如果 sync 跳过了被锁住的 rollout 文件，就告诉我哪些文件被跳过了，并提醒我稍后再补跑一次 `codex-threadkeeper sync`。
...
- 只想查看状态：`codex-threadkeeper status`
- 当前 provider 不变，只修复历史会话可见性：`codex-threadkeeper sync`
- 切 provider 并同步历史：`codex-threadkeeper switch openai`
- 安装桌面双击启动器：`codex-threadkeeper install-windows-launcher`
- 回滚误操作：`codex-threadkeeper restore <backup-dir>`
...
- `codex-threadkeeper install-windows-launcher`
  - 默认在桌面生成两个文件
  - `Codex Threadkeeper.vbs`：双击后隐藏执行，结束后弹窗显示结果
  - `Codex Threadkeeper.cmd`：可见控制台版本，方便排错
  - `--dir <path>` 可改安装目录
...
- 自动清理和手动清理都只会处理 `backups_state/threadkeeper` 下由本工具创建的备份
- 旧的 `provider-sync` 备份不会再被自动发现，但只要你显式传入旧目录路径，`codex-threadkeeper restore <backup-dir>` 仍然可以恢复
- `Codex Threadkeeper.vbs` 依赖 `codex-threadkeeper` 命令本身已经可用
